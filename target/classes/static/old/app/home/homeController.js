amadeusounds.controller('HomeController', ['$http', '$rootScope', '$scope', '$stateParams', 'HomeService', 'PlayerService',
    function ($http, $rootScope, $scope, $stateParams, HomeService, PlayerService) {
        ...
        $scope.postRating = function (songId, rating) {
            //alert("ZAPISAN e glasot - " + songId + " - " + rating);

            return $http({
                    method: 'POST',
                    url: 'http://localhost:8080/api/admin/songs/' + songId + '/rating/',
                    dataType: 'json',
                    data: {
                        rating: rating
                    }
